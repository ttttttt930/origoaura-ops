...
export interface TaxParams {
  ...
  // ---- 一般纳税人强制登记（ADR-13 / 增值税法第九条 + 税务总局公告 2026 年第 2 号）----
  /**
   * 强制登记标准：连续不超过 12 个月（或 4 个季度）经营期内累计
   * **应征增值税销售额（不含税）** 超过该值，即须登记为一般纳税人。
   * 自然人、以及不经常发生应税交易的非企业单位除外 —— 有限公司不适用例外。
   */
  generalRegThreshold: number;     // 5_000_000
  /** 临界预警比例：滚动累计达到阈值的该比例即预警（提前准备进项专票与登记） */
  thresholdWarnRatio: number;      // 0.8
  /** 逾期未登记的滞纳金日费率（按日万分之五，年化约 18.25%） */
  latePenaltyDailyRate: number;    // 0.0005
}
